...
export async function GET() {
  try {
    const response = await fetch(`${backendUrl()}/api/analytics/overview`, { cache: 'no-store' });
    const text = await response.text();
    let data: unknown = {};
    try { data = text ? JSON.parse(text) : {}; } catch { data = { error: 'Invalid backend response' }; }
    return NextResponse.json(data, { status: response.status });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Analytics unavailable.' }, { status: 503 });
  }
}
